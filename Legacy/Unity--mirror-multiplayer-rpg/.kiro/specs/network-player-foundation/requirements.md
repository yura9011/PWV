...
Sistema fundacional de red y movimiento de jugadores para "The Ether Domes", un Micro-MMORPG cooperativo para 1-10 jugadores. Este módulo establece la arquitectura híbrida Host-Play/Servidor Dedicado usando **Mirror Networking**, permitiendo que múltiples jugadores se conecten y muevan sus personajes en un mundo compartido.

## Glossary

- **NetworkManager**: Componente central de Mirror que gestiona conexiones, spawning de jugadores y sincronización de red.
...
- **NetworkBehaviour**: Clase base de Mirror para scripts que requieren funcionalidad de red.
- **NetworkIdentity**: Componente que marca un GameObject para sincronización en red.
- **isLocalPlayer**: Propiedad que indica si el cliente local tiene autoridad sobre un NetworkBehaviour.
- **isOwned**: Propiedad que indica si el cliente local es dueño del objeto de red.
- **Player_Prefab**: Prefab del personaje jugador con componentes de red configurados.
- **SyncVar**: Atributo de Mirror para sincronizar variables automáticamente del servidor a clientes.
- **Command**: Método que se ejecuta en el servidor, llamado desde el cliente.
- **ClientRpc**: Método que se ejecuta en todos los clientes, llamado desde el servidor.
...
2. WHEN a Player_Prefab is instantiated, THE NetworkIdentity component SHALL be assigned ownership to the connecting client
...
1. WHEN a player provides movement input, THE Player_Controller SHALL only process input if isLocalPlayer is true
2. WHEN isLocalPlayer is false, THE Player_Controller SHALL ignore local input for that NetworkBehaviour
3. WHEN a player moves their character, THE position SHALL synchronize to all other clients via Command/ClientRpc
