...
namespace EtherDomes.Combat
{
    ...
    /// Uses Unity Input System for ability hotkeys (can be disabled for custom input handling).
    /// </summary>
    public class AbilitySystem : MonoBehaviour, IAbilitySystem
    {
        ...
        [SerializeField] private bool _enableInputHandling = true; // NEW: Allow disabling input
        ...
        private void Start()
        {
            // Only initialize input if enabled
            if (_enableInputHandling)
            {
            ...
            {
                Debug.Log("[AbilitySystem] Start - Input handling disabled (external control)");
            }
        }
        ...
        {
            if (_enableInputHandling && _inputActions != null && !_inputEnabled)
            {
                _inputActions.Player.Enable();
                _inputEnabled = true;
            }
        ...
        private void ExecuteAbility(AbilityData ability)
        {
            ...
            if (ability.IsPullEffect && target != null)
            {
                ...
                {
                    var networkObject = component.GetComponent<Unity.Netcode.NetworkObject>();
            ...
            if (ability.CCType != CCType.None && target != null)
            {
                ...
                {
                    var networkObject = ccTargetComponent.GetComponent<Unity.Netcode.NetworkObject>();
                    if (networkObject != null)
                    {
                        targetId = networkObject.NetworkObjectId;
                    }
