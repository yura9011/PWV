...
namespace EtherDomes.Editor
{
    public static class NetworkUISetupCreator
    {
        ...
        private static void CreateCameraSilent()
        {
            ...
            // Add OnlinePlayerCamera if not present
            var cameraController = mainCamera.GetComponent<EtherDomes.Camera.OnlinePlayerCamera>();
            if (cameraController == null)
            {
                mainCamera.gameObject.AddComponent<EtherDomes.Camera.OnlinePlayerCamera>();
                Debug.Log("[Setup] Added OnlinePlayerCamera to Main Camera");
            }
        }
