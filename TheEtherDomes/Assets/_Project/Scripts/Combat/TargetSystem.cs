...
namespace EtherDomes.Combat
{
    ...
    public class TargetSystem : MonoBehaviour, ITargetSystem
    {
        ...
        {
            // Find local player using Mirror's NetworkIdentity
            var networkIdentities = FindObjectsByType<Mirror.NetworkIdentity>(FindObjectsSortMode.None);
            foreach (var netId in networkIdentities)
            {
                if (netId.isLocalPlayer)
                {
                    return netId.transform;
                }
            }
            return null;
        }
