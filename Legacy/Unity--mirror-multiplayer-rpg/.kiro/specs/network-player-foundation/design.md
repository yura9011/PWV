...
graph TB
    ...
    subgraph "Network Layer - Mirror"
        NM[NetworkManager]
        CA[Connection Approval<br/>Authenticator]
        NI[NetworkIdentity]
        NB[NetworkBehaviour]
    end
...
sequenceDiagram
    ...
    C->>NM: StartClient(IP)
    NM->>Auth: OnAuthRequestMessage(payload)
    Auth->>PS: LoadCharacterData()
    PS-->>Auth: CharacterData (encrypted)
    Auth->>Auth: Decrypt & Validate
    alt Valid Data
        Auth-->>NM: ServerAccept(conn)
        NM->>NM: OnServerAddPlayer()
        NM-->>C: Connection Success
    else Invalid Data
        Auth-->>NM: ServerReject(conn)
...
Wrapper sobre NetworkManager de Mirror que expone una API simplificada para gestión de sesiones.
...
public interface INetworkSessionManager
{
    ...
    // State
    bool IsHost { get; }      // NetworkServer.active && NetworkClient.active
    bool IsClient { get; }    // NetworkClient.active && !NetworkServer.active
    bool IsServer { get; }    // NetworkServer.active && !NetworkClient.active
    int ConnectedPlayerCount { get; }  // NetworkServer.connections.Count
    int MaxPlayers { get; }   // Returns 10
}
```

### ConnectionApprovalHandler (Mirror Authenticator)

Implementa la validación de datos de personaje usando el sistema de Authenticator de Mirror.

```csharp
public interface IConnectionApprovalHandler
{
    // Validation
    ConnectionApprovalResult ValidateConnectionRequest(NetworkConnectionToClient conn, AuthRequestMessage msg);
...
// Mirror message for authentication
...
Controlador de movimiento con verificación de ownership usando Mirror.
...
public interface IPlayerController
{
    ...
    bool IsLocalPlayer { get; }  // isLocalPlayer
    bool IsOwned { get; }        // isOwned
...
### NetworkPayload (Mirror Messages)
...
// Character creation message (existing pattern in project)
public struct CreateCharacterMessage : NetworkMessage
{
    public int ID;
    public string Name;
    public byte[] CharacterPayload;
}
```
...
- Host: IsHost=true, IsClient=false, IsServer=false (NetworkServer.active && NetworkClient.active)
- Client: IsHost=false, IsClient=true, IsServer=false (NetworkClient.active && !NetworkServer.active)
- DedicatedServer: IsHost=false, IsClient=false, IsServer=true (NetworkServer.active && !NetworkClient.active)
...
*For any* PlayerController instance and any movement input, if isLocalPlayer is false, the input SHALL be ignored and CurrentVelocity SHALL remain unchanged.
...
### Network Errors (Mirror)

| Error Scenario | Handling Strategy |
|----------------|-------------------|
| Connection timeout | OnClientDisconnect with DisconnectReason, allow retry |
| Host unreachable | OnClientError with TransportError, show "Host not found" |
| Host disconnects mid-session | OnClientDisconnect, clean up local state |
| Max players reached | Reject in OnServerConnect, send error message |
| Invalid IP format | Validate before StartClient(), show input error |
...
1. **NetworkSessionManager Tests**
   - Verify StartHost() sets NetworkServer.active && NetworkClient.active
   - Verify StartClient() with valid IP attempts connection
   - Verify StartServer() sets headless mode
   - Verify StopHost()/StopClient()/StopServer() cleans up resources
...
3. **PlayerController Tests**
   - Test movement with isLocalPlayer=true processes input
   - Test movement with isLocalPlayer=false ignores input
...
1. **Property 3: Ownership Input Processing**
   - Generate random movement inputs and isLocalPlayer states
   - Verify input ignored when isLocalPlayer=false
