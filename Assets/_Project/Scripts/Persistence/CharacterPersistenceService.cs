...
namespace EtherDomes.Persistence
{
    ...
    public class CharacterPersistenceService : ICharacterPersistenceService
    {
        ...
        /// <summary>
        /// Verifies the integrity hash stored in the character data.
        /// This ensures the data hasn't been tampered with since it was saved.
        /// </summary>
        /// <param name="data">The character data to verify</param>
        /// <param name="jsonBytes">The original JSON bytes (before IntegrityHash was set)</param>
        /// <returns>True if hash matches, false if tampered or invalid</returns>
        public bool VerifyIntegrityHash(CharacterData data, byte[] jsonBytes)
        {
            if (data == null || jsonBytes == null || jsonBytes.Length == 0)
                return false;

            if (data.IntegrityHash == null || data.IntegrityHash.Length == 0)
            {
                Debug.LogWarning("[CharacterPersistence] No integrity hash stored in character data");
                return true; // Allow legacy data without hash
            }

            return _encryption.VerifyHash(jsonBytes, data.IntegrityHash);
        }

        /// <summary>
        /// Full validation including hash verification.
        ...
        private bool ValidateStatRanges(CharacterData data)
        {
            ...
            {
                Debug.LogWarning($"[CharacterPersistence] Invalid experience: {data.Experience}");
                return false;
            }
            ...
            if (stats != null)
            {
                ...
                if (stats.Strength < 0 || stats.Strength > maxStatValue ||
                    stats.Intellect < 0 || stats.Intellect > maxStatValue ||
                    stats.Stamina < 0 || stats.Stamina > maxStatValue ||
                    stats.AttackPower < 0 || stats.AttackPower > maxStatValue * 2 ||
                    stats.SpellPower < 0 || stats.SpellPower > maxStatValue * 2)
                {
