...
namespace EtherDomes.Classes
{
    ...
    public class ClassSystem : MonoBehaviour, IClassSystem
    {
        ...
        {
            // Use explicit int cast to avoid duplicate pattern issues with enum aliases
            return (int)spec switch
            {
                (int)Specialization.CruzadoTank => SpecRole.Tank,      // Protection alias
                (int)Specialization.ProtectorTank => SpecRole.Tank,   // ProtectionPaladin alias
                (int)Specialization.BerserkerDPS => SpecRole.DPS,     // Arms, Retribution, Assassination, Combat aliases
                (int)Specialization.ArqueroDPS => SpecRole.DPS,       // BeastMastery, Marksmanship aliases
                (int)Specialization.MaestroElementalDPS => SpecRole.DPS, // Fire, Frost, Destruction aliases
                (int)Specialization.CaballeroRunicoDPS => SpecRole.DPS,  // Blood, Unholy, FrostDK aliases
                (int)Specialization.ClerigoHealer => SpecRole.Healer,    // Holy, HolyPaladin aliases
                (int)Specialization.MedicoBrujoHealer => SpecRole.DPS,   // Shadow, Affliction aliases (DPS specs)
                _ => SpecRole.DPS
            };
        }
